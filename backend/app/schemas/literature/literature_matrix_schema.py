...
class LiteratureMatrixResponse(BaseModel):
    project_id: int
    papers: List[LiteratureMatrixRow] = Field(default_factory=list)
    cross_paper_findings: List[str] = Field(default_factory=list)
    common_methods: List[str] = Field(default_factory=list)
    common_limitations: List[str] = Field(default_factory=list)
    potential_research_gaps: List[str] = Field(default_factory=list)
    evidence_note: str = (
        "Matrix insights are generated from the selected paper information. "
